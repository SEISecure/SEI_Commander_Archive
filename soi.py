...
dns_name = u'soi-821-1.nrlssc.org'
...
def readDetection():
    cursor = connection.cursor()
    cursor.execute("select timestamp, target_x, target_y, target_z from detections order by timestamp asc limit 1;")
    position = cursor.fetchone()
    return position    

def readPositionData():
    cursor = connection.cursor()
    cursor.execute("select px, py, pz from positiondata order by timestamp asc limit 1;")
    position = cursor.fetchone()
    return position    
...
def onTaskingPackage(*args, **kargs):
    ...
    print ("packageId: " , packId, "  service: ", message['pedigree']['service']['name'])
    '''
    if message['pedigree']['service']['name'] == "SQUAD COMMAND":
        ...
        for polygon in polygons:
            ...
            if grID == "firing_area":
                ...
                # send only the center
            elif grID == "target_area":
                waypointsTarget = polygon['points']
                print ("waypointsTarget: " , waypointsTarget)
                print ("lat0: ",waypointsTarget[0][0])

        MoveAverageLat = str(round((waypointsMove[0][0] + waypointsMove[1][0] + waypointsMove[2][0] + waypointsMove[3][0]) / 4, 6))
        MoveAverageLong = str(round((waypointsMove[0][1] + waypointsMove[1][1] + waypointsMove[2][1] + waypointsMove[3][1]) / 4, 6))
        MoveCoordinates = MoveAverageLat + "," + MoveAverageLong
        ...
        for behavior in behaviors1:
            bhID = behavior['behaviorId']
            if bhID.__contains__("attack"):
                unitID = behavior['forces'][0]
                print ("unitID: " , unitID)
        
        DBSaveTask("attack", unitID, MoveCoordinates, TargetCoordinates)
        '''
...
if __name__ == '__main__':
    ...
    client.publish(HASHES['assetRegistration'], "nibbler-1", ASSET_REGISTRATION, -77, 32, present_time)
    #client.publish(HASHES['track'], "nibbler-1", TRACK, 1, 1, present_time)
    client.publish(HASHES['taskingPackage'], "nibbler-1", TASKING_PACKAGE, 1, 1, present_time)
    while True :
        ...
        lat = pos[1] % 90
        long = pos[0] % 180
        ASSET_STATUS['location']['position'][0] = long
        ASSET_STATUS['location']['position'][1] = lat
        ASSET_STATUS['timestamp'] = now_time
        
        det = readDetection()
        print("detection time: ", det[0], "long: ", det[1], "lat: ", det[2])
        #time1 = time.strptime(str(det[0]), '%H:%M:%S')
        #print(time1)
        #time2 = time.mktime(time1)
        #print(time1)
        #originTime = time.mktime(time.strptime(str(det[0]), '%H:%M:%S'))
        #originTime = int(originTime1.time())
        #originTime = now_time  #det[0]
        DETECTION_MINIMUM['timeline']['created'] = now_time
        DETECTION_MINIMUM['entity']['location']['position']['coordinates'][0] = det[1] % 180
        DETECTION_MINIMUM['entity']['location']['position']['coordinates'][1] = det[2] % 90
        print('DETECTION_MINIMUM:  ', DETECTION_MINIMUM)

        client.publish(HASHES['assetStatus'], "nibbler-1", ASSET_STATUS, lat, long, now_time)
        #client.publish(HASHES['detectionMinimum'], "nibbler-1", DETECTION_MINIMUM, lat, long, now_time)
        #client.publish(HASHES['taskingPackage'], "nibbler-1", TASKING_PACKAGE, 1, 1, now_time)
        time.sleep(5)
