...
async def runner():
    ...
    while(True): 
            ...
            ft3Position = updateFireTeam("ft3", ft3Position, 4)
            
            ft1Size, ft1_avgX, ft1_avgZ = fireTeamAvgXZ(ft1Position)
            ft2Size, ft2_avgX, ft2_avgZ = fireTeamAvgXZ(ft2Position)
            ft3Size, ft3_avgX, ft3_avgZ = fireTeamAvgXZ(ft3Position)
            
            
            allSeenEnemies = []
            initialized = False
            #get all the enemies that the ft1 team sees
            for soldier in ft1Position:
                cursor.execute("select seenblufor from enemyVisibility where opfor='"+ str(soldier) + "';")
            ...
            UAVPosition, SUGVPosition, UGVPosition, mgPosition, enSoldierPosition = updateEnemy(allSeenEnemies)
            ft1nextPosReward = {}
            ft2nextPosReward = {}
            ft3nextPosReward = {}
            ...
            while(True):
                ...
                tempft3X = ft3nextX
                tempft3Z = ft3nextZ
                ft1Position = ftRandomMovement(ft1Position, tempft1X, tempft1Z)
                ft2Position = ftRandomMovement(ft2Position, tempft2X, tempft2Z) 
                ft3Position = ftRandomMovement(ft3Position, tempft3X, tempft3Z)               
                nextPosReward = 0.0
                nextPosRewardft1 = 0.0
                nextPosRewardft2 = 0.0
                nextPosRewardft3 = 0.0
                enemyLocation = {}
                UGVPosition = futureLocationRandomize(UGVPosition, 20)
                UAVPosition = futureLocationRandomize(UAVPosition, 20)
                SUGVPosition = futureLocationRandomize(SUGVPosition, 20)
                mgPosition = futureLocationRandomize(mgPosition, 20)
                enSoldierPosition = futureLocationRandomize(enSoldierPosition, 20)

                
                for t in range(0,3):
                    
                    
                    ft1var = ftvariables(ft1Position, ft2Position, ft3Position, UAVPosition, SUGVPosition, UGVPosition, mgPosition, enSoldierPosition)
                    ft2var = ftvariables(ft2Position, ft3Position, ft1Position, UAVPosition, SUGVPosition, UGVPosition, mgPosition, enSoldierPosition)
                    ft3var = ftvariables(ft3Position, ft1Position, ft2Position, UAVPosition, SUGVPosition, UGVPosition, mgPosition, enSoldierPosition)
                    inputVar = []
                    for y in range(0,len(ft1var)):
                        inputVar.append(float(ft1var[y]))
                        
                    for y in range(0,len(ft2var)):
                        inputVar.append(float(ft2var[y]))
                    
                    for y in range(0,len(ft3var)):
                        inputVar.append(float(ft3var[y]))
                    input = np.array([inputVar])
                    modelOutput = nueralNetwork(input)
                    #print(str(modelOutput[0,0].item()))
                    ft1SizePer = float(str(modelOutput[0,0].item()))
                    ft2SizePer = float(str(modelOutput[0,1].item()))
                    ft3SizePer = float(str(modelOutput[0,2].item()))
                    ...
                    tempdelta = tempTime - SimulationStart
                    ElapsedTime = tempdelta.seconds
                    #nextPosReward -= (1- (ft1Size+ft2Size+ft3Size)/13)
                    #print(ft1Size+ft2Size+ft3Size)
                    #nextPosReward += killedEnemy
                    denominator = (780 -779 *(ElapsedTime/600))
                    if(denominator < 1):
                        denominator = 1
                    
                    ft1temp = 1-(distanceToFOB(tempft1X, tempft1Z))/denominator
                    #if(ft1temp < 0):
                    #    ft1temp = 0
                    if(distanceToFOB(tempft1X, tempft1Z) < 30):                        
                        nextPosReward += 12 * (ft1temp) * (ft1SizePer)
                    else:
                        nextPosReward += (ft1temp) * (ft1SizePer)
                    
                    ft2temp = 1-(distanceToFOB(tempft2X, tempft2Z))/denominator
                    #if(ft2temp < 0):
                    #    ft2temp = 0
                    if(distanceToFOB(tempft2X, tempft2Z) < 30):                        
                        nextPosReward += 12 * (ft2temp) * (ft2SizePer) 
                    else:
                        nextPosReward += (ft2temp) * (ft2SizePer)   
                    
                    ft3temp = 1-(distanceToFOB(tempft3X, tempft3Z))/denominator
                    #if(ft3temp < 0):
                    #    ft3temp = 0    
                    if(distanceToFOB(tempft2X, tempft2Z) < 30):                        
                        nextPosReward += 12 * (ft3temp) * (ft3SizePer) 
                    else:
                        nextPosReward += (ft3temp) * (ft3SizePer)       
            ...
            if(len(ft1nextPosReward) > 0):
                ...
                print(newXZ)
                if(newXZ[0] != ''):                    
                    soldierid = continueAssault("ft1", float(newXZ[0]), float(newXZ[1]), soldierid)
            ...
            if(len(ft2nextPosReward) > 0):
                ...
                print(newXZ)
                if(newXZ[0] != ''):
                    soldierid = continueAssault("ft2", float(newXZ[0]), float(newXZ[1]), soldierid)
            ...
            if(len(ft3nextPosReward) > 0):
                ...
                print(newXZ)
                if(newXZ[0] != ''):
                    soldierid = continueAssault("ft3", float(newXZ[0]), float(newXZ[1]), soldierid)
...
def updateFireTeam(ft, ftPosition, ftSize):
    ...
    global soldierDict
    cursor = connection.cursor()
    cursor.execute("select unit_name, health, px, pz from opforpositiondata where unit_name like '" + ft + "%' order by timestamp desc limit "+ str(ftSize) + ";")
...
def updateEnemy(enemyPosition):
    ...
    for enemy in enemyPosition:
        cursor.execute("select health, px, pz, vx, vz from blueforpositiondata where unit_name = '" + enemy + "' order by timestamp desc limit 1;")
        e = cursor.fetchone()
