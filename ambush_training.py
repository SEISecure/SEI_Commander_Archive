...
async def runner():
    ...
    #elevationDict = FileIO.getElevationFromFileVBS4("elevation_db_village_control")
    #terrainVariablesToFile('Ambush_Village_Control', 1000160, 1004070, 999910, 1003490, "Village", "terrainType_db_village_control" )
    #print("Done")
    ...
    #print(ft1Lethality)
    #saveLethalityPredicitionAtAmbushZoneToFile('Ambush_Village_Control', 1000160, 1004070, 999910, 1003490)
    #print("Done")
    ...
    numOfOpFor = database_connect.getNumberofOpForceUnits()
    numOfBlueFor = database_connect.getNumberofBlueForceUnits()  
    ...
    if("Ft1" not in missionType and "Ft2" not in missionType):
        ...
        twoFireTeams()
    '''
    if("Village" in missionType):
        print("Launching Village-Control") 
        opforAmbush(numOfOpFor,numOfBlueFor)     
...
def opforAmbush(numOfOpFor, numOfBlueFor):
    global model      
    random.seed()
    commandNotIssued = True
    commandNotIssuedTWO = True
    model = loadNueralNetwork.loadModel()
    opforleastLethality, opforLethalArray = getPredictedLethalityFromFile('Ambush_Village_Control')

    opforAmbushSiteNumber=0
    if(forestMap):
        opforAmbushSiteNumber = len(opforLethalArray)*0.3
    else:
        opforAmbushSiteNumber = len(opforLethalArray)*.1
    #sorted array of lethalitys descending order
    #len/4
    #opforSentXLocation = 0
    #opforSentZLocation = 0
    opforpredictedLethality = random.randint(0,int(opforAmbushSiteNumber))
    
    
    while  commandNotIssued :
        
        for X , Z in opforleastLethality:
            lethality = opforleastLethality[X,Z]
            opforSentXLocation = X
            opforSentZLocation = Z

            index = round(len(opforleastLethality)/2)+opforpredictedLethality
            predLethal = opforLethalArray[int(index)] 
            ...
            if(predLethal== lethality[0]):
                print (str(X)+','+str(Z))
                if  X > 999910 and X <1000150 and Z <1003800:
                    BuddyTeam1 = database_connect.getOpforBuddyTeam("bd1", 2)
                    commandBuddyTeam(BuddyTeam1,X,Z)
                    BuddyTeam2 = database_connect.getOpforBuddyTeam("bd2", 2)
                    commandBuddyTeam(BuddyTeam2,X+10,Z)
                    BuddyTeam3 = database_connect.getOpforBuddyTeam("bd3", 2)
                    commandBuddyTeam(BuddyTeam3,X-10,Z)
                    commandNotIssued=False
                    print("ft1 en route")
                    break
                  
                elif X < 1000030 and X > 999910 and Z > 1003800 and Z <1003990:
                    BuddyTeam1 = database_connect.getOpforBuddyTeam("bd1", 2)
                    commandBuddyTeam(BuddyTeam1,X,Z)
                    BuddyTeam2 = database_connect.getOpforBuddyTeam("bd2", 2)
                    commandBuddyTeam(BuddyTeam2,X+10,Z)
    ...
    global forestDict
    global elevationDict
    forestDict = FileIO.getForestFromTerrainFile('terrainType_db_village_control')
    elevationDict = FileIO.getElevationFromFileVBS4("elevation_db_village_control")
    opforKillPercentage = calculateKillPercentages(numOfBlueFor,numOfOpFor)
    #print(opforKillPercentage)
    

    
    openTerrain, closed, neighbors, DToPath, ElevationChanged = getPositionDataAtAmbushFromFile('Ambush_Village_Control', roundup(opforSentXLocation),roundup(opforSentZLocation))
    #print(openTerrain, closed, neighbors, DToPath, ElevationChanged)
    #exit(0)
    saveToTrainingFile('Ambush_Village_Control_ambushTraining', opforKillPercentage, openTerrain, closed, neighbors, DToPath, ElevationChanged)
    print("Training Model Updated")
    generateUpdatedLethality('Ambush_Village_Control', 1000160, 1004070, 999910, 1003490)
...
def generateOpForceBDTeamOutput(BuddyTeam, BlueForceSize, opForSize):
    opforTeam = database_connect.getOpforBuddyTeam(BuddyTeam, 2)
    ...
    opfor = opforTeam[0]   
    opforAlly = opforTeam[1]
    DNA = StateSpaceCalc.distanceToNearestOpForAlly(opfor[0], opForSize)  
    DNE, NearestEType = StateSpaceCalc.distanceBetweenNearestOpForEnemyandType(opfor[0], BlueForceSize) 
    AllySectors = []
    Sol2Sector = ReadTerrainFile.sectorOfOpForAlly(opfor[0], opforAlly[0]) 
    if(Sol2Sector != -1):
    ...
    while i < len(soldiers):
        EnemySector = ReadTerrainFile.sectorOfOpForEnemy(opfor[0], soldiers[i][0])  
        i = i +1     
        if(EnemySector != -1):
            EnemySectors.append(EnemySector)
    '''
    ...
    collectDataNSaveToFileOpfor(opfor, opforAlly, DNA, DNE, NearestEType, AllySectors, EnemySectors,BlueForceSize)   

    opfor = opforTeam[1]   
    opforAlly = opforTeam[0]  
    DNA = StateSpaceCalc.distanceToNearestOpForAlly(opfor[0], opForSize)  
    DNE, NearestEType = StateSpaceCalc.distanceBetweenNearestOpForEnemyandType(opfor[0],BlueForceSize) 
    AllySectors = []
    Sol2Sector = ReadTerrainFile.sectorOfOpForAlly(opfor[0], opforAlly[0]) 
...
def collectDataNSaveToFileOpfor(opfor, opforAlly, DNA, DNE, NearestEType, AllySectors, EnemySectors, blueForceSize):  
    global forestDict
    global elevationDict
    global outputPathOp
    
    Elevation = elevationDict[roundup(float(opfor[2])), roundup(float(opfor[3]))]            
    ElevationDiff = StateSpaceCalc.elevationDifFromNearestOpForEnemy(opfor[0],elevationDict, blueForceSize)
    PercentForestAtEnemy = StateSpaceCalc.getForestAtNearestOpForEnemyLocation(opfor[0], forestDict, blueForceSize )
    PercentForestAtSol = StateSpaceCalc.getForestAtOpForLocation(opfor[0], forestDict)
    PercentForestAtSector = ReadTerrainFile.percentForestAtSector([opfor[2], opfor[3]], forestDict)
    greatestAngle, smallestAngle = StateSpaceCalc.OpForAnglesOfAttack([opfor[2], opfor[3]], [opforAlly[2], opforAlly[3]], blueForceSize)


    saveToAgentDataFile(outputPathOp, opfor[6], opfor, DNA , DNE, AllySectors, EnemySectors, NearestEType, 
                        greatestAngle, smallestAngle, opfor[4], opfor[5], opfor[2], opfor[3], ElevationDiff, Elevation, opfor[1], 
...
def collectDataNSaveToFileBluefor(bluefor, blueforAlly, DNA, DNE, NearestEType, AllySectors, EnemySectors, opForceSize): 
    global forestDict
    global elevationDict
    global outputPathBlue

    Elevation = elevationDict[roundup(float(bluefor[2])), roundup(float(bluefor[3]))]            
    ElevationDiff = StateSpaceCalc.elevationDifFromNearestBlueForEnemy(bluefor[0],elevationDict, opForceSize)
    PercentForestAtEnemy = StateSpaceCalc.getForestAtNearestBlueForEnemyLocation(bluefor[0], forestDict, opForceSize)
    PercentForestAtSol = StateSpaceCalc.getForestAtBlueForLocation(bluefor[0], forestDict)
    PercentForestAtSector = ReadTerrainFile.percentForestAtSector([bluefor[2], bluefor[3]], forestDict)
    
    greatestAngle, smallestAngle = StateSpaceCalc.BlueForAnglesOfAttack([bluefor[1], bluefor[3]], [blueforAlly[2], blueforAlly[3]], opForceSize)
    
    
    saveToAgentDataFileBlue(outputPathBlue, bluefor[6], bluefor, DNA , DNE, AllySectors, EnemySectors, NearestEType,
                 greatestAngle, smallestAngle, bluefor[4], bluefor[5], bluefor[1], bluefor[2], ElevationDiff, Elevation, bluefor[1],
                  PercentForestAtEnemy, PercentForestAtSol, PercentForestAtSector[0], PercentForestAtSector[1], PercentForestAtSector[2], PercentForestAtSector[3] )
    

def generateOpForceOutput(BlueForceSize, opForSize):
    print("printing opfor data")
    generateOpForceBDTeamOutput("bd1",BlueForceSize, opForSize)
    generateOpForceBDTeamOutput("bd2",BlueForceSize, opForSize)
    generateOpForceBDTeamOutput("bd3",BlueForceSize, opForSize)
    print("Done")


def generateBlueForceOutput(BlueForceSize, OpForceSize):
    
    soldiers = database_connect.getAllBlueForceUnits(BlueForceSize);
    opfor = database_connect.getAllOpForceUnits(OpForceSize);
    '''
    ...
    while soldierIndex < len(soldiers):
        #print(soldiers[soldierIndex][0])
        blueforAlly = []
        bluefor = soldiers[soldierIndex] 
        if(soldierIndex < len(soldiers)-1):
            blueforAlly = soldiers[soldierIndex+1] 
        else:
            blueforAlly = soldiers[0]    
        DNA = StateSpaceCalc.distanceToNearestBlueForAlly(bluefor[0],BlueForceSize)  
        DNE, NearestEType = StateSpaceCalc.distanceBetweenNearestBlueForEnemyandType(bluefor[0],OpForceSize ) 
        AllySectors = []
           
        
        i = 0
        while i < len(soldiers):
            AllySector = ReadTerrainFile.sectorOfBlueForAlly(bluefor[0], soldiers[i][0])  
            i = i +1     
            if(AllySector != -1):
                AllySectors.append(AllySector)    
        
        EnemySectors = []
        
        i = 0
        while i < len(opfor):
            EnemySector = ReadTerrainFile.sectorOfBlueForEnemy(bluefor[0], opfor[i][0])  
            i = i +1     
            if(EnemySector != -1):
                EnemySectors.append(EnemySector)
        
        collectDataNSaveToFileBluefor(bluefor, blueforAlly, DNA, DNE, NearestEType, AllySectors, EnemySectors, OpForceSize)
        soldierIndex+=1
...
def opforMove(opForSize, blueForSize):
    global forestDict
    global elevationDict
    global model
    model.eval()
    ...
    enemies = database_connect.getAllBlueForceUnits(blueForSize);

    for i in range(0,opForSize,2):
        opforlead = soldiers[i-1]  
        opformember = soldiers[i]     
        DNEbd_lead, NearestETypebd_lead = StateSpaceCalc.distanceBetweenNearestOpForEnemyandType(opforlead[0], blueForSize)   
        DNEbd_member, NearestETypebd_member = StateSpaceCalc.distanceBetweenNearestOpForEnemyandType(opformember[0],blueForSize) 
        ft_leadX, ft_leadZ = float(opforlead[2]), float(opforlead[3])
        ft_memberX, ft_memberZ = float(opformember[2]), float(opformember[3])
        ft_leadHP, ft_memberHP = float(opforlead[1]), float(opformember[1])
    
        if(DNEbd_lead <= 150 or DNEbd_member <= 150):
            ft_leadXZ = {}
            ft_memberXZ ={}
            for i in range(0,30):
                newft_leadX = randomMovement(ft_leadX, 5)
                ...
                if(ft_leadHP <= 0.99):
                    ...
                    if(ft_memberHP <= 0.99):
                        DNAft_lead = StateSpaceCalc.distanceBetweenPoints(newft_leadX,newft_leadZ, newft_memberX, newft_memberZ) 
                        AllySectorft_lead = ReadTerrainFile.sectorOfOpForAllyPos([newft_leadX, newft_leadZ], [newft_memberX, newft_memberZ])     
                        greatestAngleft_lead, smallestAngleft_lead = StateSpaceCalc.OpForAnglesOfAttack([newft_leadX, newft_leadZ], [newft_memberX, newft_memberZ], blueForSize)
                    
                    
                    EnemySectors = []
                    i = 0
                    while i < len(enemies):
                        EnemySector = ReadTerrainFile.sectorOfOpForEnemy(opforlead[0], enemies[i][0])  
                        i +=1     
                        if(EnemySector != -1):
                            EnemySectors.append(EnemySector)
                    numEnmSectNorth =0
                    ...
                    ftEnemyType = 0
                    if(NearestETypebd_lead != "Rifleman"):
                        ftEnemyType = 1
                    newDNEft_lead = StateSpaceCalc.distanceBetweenNearestOpForEnemyWPos([newft_leadX, newft_leadZ],opforlead[0],blueForSize)  
                    DNEft_lead =  newDNEft_lead
                    Elevationft_lead = elevationDict[roundup(newft_leadX), roundup(newft_leadZ)]            
                    ElevationDiff_ft_lead = StateSpaceCalc.elevationDifFromNearestOpForEnemyPos([newft_leadX, newft_leadZ],opforlead[0],elevationDict,blueForSize)
                    PercentForestAtEnemyft_lead = StateSpaceCalc.getForestAtNearestOpForPosEnemyLocation([newft_leadX, newft_leadZ], forestDict, blueForSize)
                    PercentForestAtSolft_lead = StateSpaceCalc.getForestAtOpForPosLocation([newft_leadX, newft_leadZ], forestDict)
                    PercentForestAtSectorft_lead = ReadTerrainFile.percentForestAtSector([newft_leadX, newft_leadZ], forestDict)
                    #predict outcome 
                    ftlead_input = np.array([[round(DNAft_lead), round(DNEft_lead),
                ...
                if(ft_memberHP <= 0.99):
                    ...
                    if(ft_leadHP <= 0.99):
                        DNAft_member = StateSpaceCalc.distanceBetweenPoints(newft_leadX,newft_leadZ, newft_memberX, newft_memberZ) 
                        AllySectorft_member = ReadTerrainFile.sectorOfOpForAllyPos([newft_memberX, newft_memberZ], [newft_leadX, newft_leadZ])     
                        greatestAngleft_member, smallestAngleft_member = StateSpaceCalc.OpForAnglesOfAttack([newft_memberX, newft_memberZ], [newft_leadX, newft_leadZ], blueForSize)
                    ...
                    for sect in EnemySectors:
                        if sect =='1':
                            ...
                            numEnmSectWest+=1
                    ftEnemyType = 0
                    if(NearestETypebd_member != "Rifleman"):
                        ftEnemyType = 1
                    newDNEft_member = StateSpaceCalc.distanceBetweenNearestOpForEnemyWPos([newft_memberX, newft_memberZ], opformember[0], blueForSize)  
                    DNEft_member =  newDNEft_member
                    Elevationft_member = elevationDict[roundup(newft_memberX), roundup(newft_memberZ)]            
                    ElevationDiff_ft_member = StateSpaceCalc.elevationDifFromNearestOpForEnemyPos([newft_memberX, newft_memberZ],opforlead[0],elevationDict,blueForSize)
                    PercentForestAtEnemyft_member = StateSpaceCalc.getForestAtNearestOpForPosEnemyLocation([newft_memberX, newft_memberZ], forestDict, blueForSize)
                    PercentForestAtSolft_member = StateSpaceCalc.getForestAtOpForPosLocation([newft_memberX, newft_memberZ], forestDict)
                    PercentForestAtSectorft_member = ReadTerrainFile.percentForestAtSector([newft_memberX, newft_memberZ], forestDict)
                    #predict outcome 
                    #predict outcome 
...
def calculateKillPercentagesFt2():    
    ...
    while(True):
        ...
        generateOpForceBDTeamOutput("bd2")    
...
def calculateKillPercentagesFt1(missionType):  
    ...
    while(True):
        ...
        generateOpForceBDTeamOutput("bd1")    
        generateBlueForceOutput()
        ft1Move()
...
def calculateKillPercentages(numOfBlueFor, numOfOpFor):
    Count = 0  
    #BlueForceRemaining = numOfBlueFor
    Ft1Killed = 0
    Ft1KillPercentage = 0     
    while(True):
        Count +=1
        ...
        #print("Kill Percentage", Ft1KillPercentage) 
        generateOpForceOutput(numOfBlueFor,numOfOpFor )    
        generateBlueForceOutput(numOfBlueFor,numOfOpFor)
        opforMove(numOfOpFor, numOfBlueFor)
        # and float(ft2lead[0]) > 0.99 and float(ft2member[0]) > 0.99
        
        #print(blueSol1[0] + ',' + blueSol2[0] + ',' + blueSol3[0] + ',' + blueSol4[0])
        if(isOpForDead(numOfOpFor) or isBlueForDead(numOfBlueFor)):
            soldiers = database_connect.getAllBlueForceUnits(numOfBlueFor);
            opfor = database_connect.getAllOpForceUnits(numOfOpFor);
            for soldier in soldiers:              
                distanceFromOpFor = distanceBetweenUnits(opfor[0][0], soldier[0])
                if(distanceFromOpFor <= 200):         
                    if(float(soldier[1]) > 0.90 ):
                        Ft1Killed+=1
            print(Ft1Killed)    
            Ft1KillPercentage = Ft1Killed / numOfBlueFor
            break
        time.sleep(5)
    return Ft1KillPercentage


#create function to check if all opfor units are alive
def isOpForDead(OpForSize):
    soldiers = database_connect.getAllOpForceUnits(OpForSize);
    deathCounter = 0
    for soldier in soldiers:
        if(float(soldier[1]) > 0.92):
            deathCounter +=1
    if deathCounter == OpForSize:      
        return True  
   
    return False
#create function to check if all bluefor units are alive
def isBlueForDead(BlueForSize):
    soldiers = database_connect.getAllBlueForceUnits(BlueForSize);
    deathCounter = 0
    for soldier in soldiers:
        if(float(soldier[1]) > 0.92):
            deathCounter +=1
    if deathCounter == BlueForSize:      
        return True  
   
    return False

def commandBuddyTeam(BuddyTeam, X, Z):
    soldierpositionString = "'" + str(round(X)) + ";" + "3.9" + ";" + str(round(Z))  + "'"
    BdLead = BuddyTeam[0]
    database_connect.soldierTeamMoveCommand(BdLead[0], soldierpositionString)
...
def terrainVariablesToFile(fileName, Xmax, Zmax, Xmin, Zmin, missionType, terrainType_db):
    global TDict
    global elevationDict
    #global missionType
    XSet,ZSet, TDict = ReadTerrainFile.readTerrainFile(terrainType_db)
...
def calculateLethalityAtAmbushFromFile(fileName, X, Z): 
    ...
    with open(fileName+'.csv', 'rt') as f:
        ...
        for line in results:
            #print(line)
            lethality = linearRegression.predictLethalityAtPos(float(line[2]),float(line[3]), float(line[4]), float(line[5]), float(line[6]),fileName)[0,0]
