...
def recon():
    ...
    if reward > bestReward:
        ...
        bestSoldierMove = copy.deepcopy(initalTeamMove)
        dangerArea, assaultedArea = FileIO_Arma.getDangerAreaFromFile("Prediction-50")
     '''  
    dangerArea, assaultedArea = FileIO_Arma.getDangerAreaFromFile("Prediction-50")
    while True: 
        loopCounter = 0      
        ...
        while i <  len(dangerArea):
            ...
            #assaultedArea[i+1] = isAtDangerArea(x[i+1],y[i+1], assaultedArea[i])     
            print("Current Positions", i, i+1)
            if assaultedArea[i] == False and assaultedArea[i + 1] == False:         
                for l in range(0, halfLoc):
                    for j in range(halfLoc, len(soldierName)):
                        if soldierName[l] == soldierName[j]:
                            continue
                        else:
                            #firstSoldier = tempSoldierLoc[soldierName[i]]
                            #secondSoldier = tempSoldierLoc[soldierName[j]]
                            database_connect_Arma.moveUnit(x[i], y[i], 2, soldierName[l])
                            database_connect_Arma.moveUnit(x[i+1], y[i+1], 2, soldierName[j])
            ...
            if i != 20:
                ...
                assaultedArea[i+1] = isAtDangerArea(x[i+1],y[i+1], assaultedArea[i])     
                if assaultedArea[i] == False and assaultedArea[i + 1] == False:         
                    for l in range(0, len(soldierName)):
                        for j in range(l+1, len(soldierName)):
                            if soldierName[i] == soldierName[j]:
                                continue
                            else:
                                #firstSoldier = tempSoldierLoc[soldierName[i]]
                    ...
                    for l in range(0, len(soldierName)):
                        for j in range(l+1, len(soldierName)):
                            if soldierName[i] == soldierName[j]:
                                ...
                                #secondSoldier = tempSoldierLoc[soldierName[j]]
                                database_connect_Arma.moveUnit(x[i], y[i], 2, soldierName[l])
                                database_connect_Arma.moveUnit(x[i], y[i], 2, soldierName[j])
            '''
        ...
        dangerArea, assaultedArea = FileIO_Arma.getDangerAreaFromFile("Prediction-50")
        if(loopCounter % 2 != 0):
            dangerArea = dangerArea.sort(reverse = True)
